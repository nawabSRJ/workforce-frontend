...
export default function Inbox({ user }) {
    ...
    const handlePlaceOrder = async (orderDetails) => {
        console.log("handlePlaceOrder called with orderDetails:", orderDetails);
        ...
        try {
            ...
            const orderData = {
                ...
                openTaskId: orderDetails.openTaskId, // Now using passed data
                amount: orderDetails.amount
            };

            console.log("Final order data:", orderData);

            // Create the order from the open task
            const res = await axios.post(`${backendURL}/create-order`, orderData);

            console.log("Order creation API response:", res.data);

            // Show success alert immediately
            window.alert('Order placed successfully!');

            if (res.data.success) {
                console.log("Order created successfully with ID:", res.data.project?._id);

                // Update the open task status to 'Accepted'
                try {
                    console.log("Updating task status to Accepted");
                ...
                const confirmMessage = {
                    ...
                    message: `Order placed successfully! The project "${orderDetails.title}" has been created. (This message is auto-generated by WorkForce)`
                };

                console.log("Sending confirmation message:", confirmMessage);

                // Send message using socket
                socket.emit("sendMessage", confirmMessage, (response) => {
                    console.log("Confirmation message sent response:", response);
                });

                // Refresh messages to show the confirmation
                setTimeout(() => {
            ...
            window.alert('Failed to place order: ' + (error.response?.data?.message || error.message || 'Unknown error'));
        }
    };
                                        ...
                                        >
                                            <div className="max-w-[80%] p-4 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 shadow-md">
                                                ...
                                                <div className="flex justify-between items-center pt-2 border-t border-gray-200 dark:border-gray-700">
                                                    ...
                                                    {!(msg.senderId !== user.id && user.type === 'Client') && (
                                                        <div className="flex gap-2">
                                                            <button
                                                                onClick={() => handlePlaceOrder(msg)}
